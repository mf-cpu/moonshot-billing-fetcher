...
def main():
    ...
    with sync_playwright() as p:
        ...
        params_raw = parsed.get("params", [""])[0]
        ...
        if params_raw:
            try:
                params_json = json.loads(unquote(params_raw))
        ...
        if isinstance(params_json, dict):
            workspace_id = (
                params_json.get("Data", {})
                .get("reqDTO", {})
                .get("filterWorkspaceId")
            )

        print("\n=== 请复制以下变量到运行环境中 ===")
        print(f"ALIYUN_BAILIAN_USAGE_URL={request.url}")
        if region:
            print(f"ALIYUN_BAILIAN_REGION={region}")
        if sec_token:
            print(f"ALIYUN_BAILIAN_SEC_TOKEN={sec_token}")
        if csrf_token:
            print(f"ALIYUN_BAILIAN_CSRF_TOKEN={csrf_token}")
        if workspace_id:
            print(f"ALIYUN_BAILIAN_WORKSPACE_ID={workspace_id}")
        print(f"ALIYUN_BAILIAN_COOKIE={cookie_header}")

        if params_json:
            print("\n=== 解析到的 params JSON（如需保留 cornerstoneParam）===")
